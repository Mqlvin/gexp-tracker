...
export function startRequesting(): void {
    requestGuildDataAndUpdatePlayers(); // call it once to start or else it'll wait 60s to run the first one
    setInterval(requestGuildDataAndUpdatePlayers, SECONDS_BETWEEN_REQUEST * 1000);
}

export async function requestGuildDataAndUpdatePlayers(): Promise<void> {
